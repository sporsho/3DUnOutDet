...
def main(args):
    ...
    with torch.no_grad():
        for pcd_file in im_idx:
            ...
            eend = time.time_ns()
            ex_time.append(eend - estart)
    # end = time.time_ns()
    # total_time = end - start
    # avg_time_per_pred = total_time / len(im_idx)
    print(f'preprocessing t time: {np.mean(pre_time) / 1000000}')
    print(f'execution t time: {np.mean(ex_time) / 1000000}')
...
#
#
# Namespace(data_dir='/var/local/home/aburai/DATA/WADS2', label_config='../configs/binary_desnow_wads.yaml', model_save_path='/var/local/home/aburai/DATA/exp_2024/bin_seg/OutDet/outdet.pt', test_output_path='/var/local/home/aburai/DATA/exp_2024/bin_seg/OutDet/outputs', model='polar', device='cuda:0', K=3, test_batch_size=1, desnow_root=None, pred_folder=None, snow_label=None, save_pred=False)
# preprocessing t time: 1.5423267995079952
# execution t time: 43.63083014391144