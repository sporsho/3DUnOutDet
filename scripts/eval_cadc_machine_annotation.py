...
def main(args):
    ...
    test_split = CADC['test']
    for seq in human_annotated_seq:
        test_split.remove(seq)

    pcms = np.zeros(shape=(2, 2, 2), dtype=np.int64)
    for seq in test_split:
        label_files = glob.glob(os.path.join(data_root, 'sequences', seq, 'machine_annotations', "*.label"))
        for lf in label_files:
            pf = lf.replace('machine_annotations', 'predictions')
